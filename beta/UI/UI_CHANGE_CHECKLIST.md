...
- [x] Restored the exact original mask image to `beta/UI/public/assets/agent-mask-original.jpg` by extracting the attached image payload from local session logs.
...
- [x] Added persistent MCP UI design profile at `mcp/UI_DESIGN_CONTEXT.json` with required tags (`coding`, `jarvis`, `program`, `matrix`, `chips`, `podsvetka`) and image references from `/beta/UI/image`.
